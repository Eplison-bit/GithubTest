...
void changeDietType(struct User *user); // New function
...
void changeDietType(struct User *user) {
    int validInput;
    do {
        validInput = 0;
        printf("\nSelect your new diet type:\n1. Keto\n2. Vegan\n3. Low Carb\n4. Low Fat\n5. Weight Watchers\nChoice: ");
        if (scanf("%d", &user->dietType) == 1 && user->dietType >= 1 && user->dietType <= 5) {
            validInput = 1;
        } else {
            printf("Invalid input. Please enter a number between 1 and 5.\n");
    ...
    if (fp) {
        ...
        fclose(fp);
        printf("Diet type updated successfully!\n");
    } else {
        printf("Error: Could not update user data file.\n");
    }
}
...
void processMeal(struct User *user, int mealType) {
    if (user->mealCount >= 20) {
        printf("Maximum meal limit reached for today (20 meals)!\n");
        return;
    }
